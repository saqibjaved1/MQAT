...
def rule_inq_nodes(nodes_set):
    """Example built-in rule: select nodes performing linear transformations.

    Return `list` of `Node`s."""
    filtered_nodes_set = []
    for n, m in nodes_set:
        cond1 = m.__class__.__name__ =="INQConv2d"
        cond2 = m.__class__.__name__ =="INQLinear"
        if cond1 or cond2:
            filtered_nodes_set.append(Node(n, m))
    return filtered_nodes_set
...
def rule_resnet_linear_nodes(nodes_set):
    """Example built-in rule: select last nodes performing linear transformations for ResNet.
    ...
    for n, m in nodes_set:
        ...
        cond2 = m.__class__.__name__ in dir(nn.modules.conv)
        if cond1 or cond2:
            if n.find("conv1") == -1 and n.find("downsample")==-1:
                filtered_nodes_set.append(Node(n, m))
    return filtered_nodes_set

def rule_resnet_activation_nodes(nodes_set):
    """Example built-in rule: select last nodes performing linear transformations for ResNet.

    Return `list` of `Node`s."""
    filtered_nodes_set = []
    for n, m in nodes_set:
        cond1 = m.__class__.__name__ in dir(nn.modules.activation)
        if cond1:
            if n.find("relu3") != -1 and n.find("layer4.1") == -1:
                filtered_nodes_set.append(Node(n, m))
    return filtered_nodes_set

def rule_ssd_linear_nodes(nodes_set):
    """Example built-in rule: select nodes performing linear transformations for SSD and exclude last layer.
...
def rule_single_block_except(nodes_set, block_name):
    """Example built-in rule: select nodes except by block (name prefix).

    Return `list` of `Node`s."""
    filtered_nodes_set = []
    for n, m in nodes_set:
        if n.find(block_name)!=-1:  # block name is a prefix of node's name
            # print("sdaa"+n)
            filtered_nodes_set.append(Node(n, m))
    return filtered_nodes_set