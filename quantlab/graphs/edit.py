...
def add_ste_after(net, nodes_set, num_levels, quant_start_epoch=0):
    for n, _ in nodes_set:
        ste_node = algo.ste.STEActivation(num_levels=num_levels, quant_start_epoch=quant_start_epoch)
        m = _get_node(net, n)
        _replace_node(net, n, nn.Sequential(OrderedDict([('func', m),('ste', ste_node)])))
...
def replace_linear_inq(net, nodes_set, num_levels, quant_init_method=None, quant_strategy='magnitude'):
    ...
    for n, _ in nodes_set:
        ...
        if m_type == 'Linear':
            ...
            bias = True if m.bias!=None else False
...
def replace_linear_inq_wide_depth(net, nodes_set, num_levels, quant_init_method=None, quant_strategy='magnitude'):
    """Replace nodes representing linear operations with INQ counterparts.
    ...
    for n, _ in nodes_set:
        ...
        # if (n.find("backbone")!=-1 and n.find("model")!=-1) or n.find("head")!=-1 or n.find("fpn")!=-1:
        if m_type == 'Linear':
            in_features = m.in_features
            out_features = m.out_features
            bias = True if m.bias!=None else False
