...
class Logbook(object):
    ...
    def __init__(self, problem, topology, exp_id,new_exp, verbose=True):
        ...
        self.config        = None
        self.exp_id        = None
        self._setup_experiment(exp_id,new_exp)
    ...
    def _setup_experiment(self, exp_id,new_exp):
        ...
        if self.is_master:
            ...
            with open(HARD_STORAGE, 'r') as fp:
                d = json.load(fp)
                # data
                HARD_HOME_DATA = os.path.join(d['data'], 'data')
                if not os.path.isdir(HARD_HOME_DATA):
                    raise FileNotFoundError('{} hard directory (data) not found: {}'.format(self.problem, HARD_HOME_DATA))
                # log
                HARD_HOME_LOGS = os.path.join(d['logs'], 'logs')
                if not os.path.isdir(HARD_HOME_LOGS):
                    raise FileNotFoundError('{} hard directory (logs) not found: {}'.format(self.problem, HARD_HOME_LOGS))
            ...
            if not os.path.isdir(dir_data):
                os.symlink(HARD_HOME_DATA, dir_data)
            dir_logs = os.path.join(DIR_PROBLEM, 'logs')
            if not os.path.isdir(dir_logs):
                os.symlink(HARD_HOME_LOGS, dir_logs)
            # get pointers to PRIVATE experiment resources
            if exp_id and new_exp is not True:
                ...
                if len(exp_folders) == 0:
                    ...
                    self.exp_id = max(int(f.replace('exp', '')) for f in exp_folders) + 1
            dir_exp = os.path.join(dir_logs, 'exp'+str(self.exp_id).rjust(__ALIGN_EXP__, '0'))
            ...
            if not os.path.isfile(private_config_file) or new_exp==True:
                # no configuration in the experiment folder: look for global one
                print(os.path.dirname(self.lib.__file__))
    ...
    def load_checkpoint(self, net, opt, lr_sched, ctrls, load, exp, cfg):
        if self.is_master:

            self.i_epoch = 0
            
            if load!=None and load!='best' and load!='last':
                # look for a checkpoint
                ckpt_list = os.listdir(self.dir_saves.replace(str(self.exp_id).rjust(__ALIGN_EXP__, '0'),str(exp).rjust(__ALIGN_EXP__, '0')))
            ...
            if len(ckpt_list) != 0:
                ...
                elif load != None :  # used in 'WHAT IF' experiments as an initial condition
                    ckpt_id = str(load).rjust(__ALIGN_EPOCHS__, '0')
                    ckpt_name = 'epoch' + ckpt_id + '.ckpt'
                    ckpt_file = os.path.join(self.dir_saves.replace(str(self.exp_id).rjust(__ALIGN_EXP__, '0'),str(exp).rjust(__ALIGN_EXP__, '0')), ckpt_name)
                    print(ckpt_file)
                else:
                    ckpt_file = None
                ...
                if (load is None or load =="best" or load == "last"):                
