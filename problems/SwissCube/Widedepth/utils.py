...
def visualize_pred(img, gt, pred, mean, std, sym_types, meshes):
    ...
    for score, cls_id, R, T in pred:
        isSym = (cls_id in sym_types)

        # GT keypoint reprojection
        pt3d = np.array(gtPoses.keypoints_3d[cls_id])
        ptCnt = len(pt3d)
        pts = np.matmul(gtPoses.K, np.matmul(R, pt3d.transpose()) + T)
        xs = pts[0] / (pts[2] + 1e-8)
        ys = pts[1] / (pts[2] + 1e-8)

        # get mesh surface points and diameter
        ms = meshes[cls_id]
        msPts = np.array(ms.vertices)
        tmp_index = np.random.choice(len(msPts), 1000, replace=True)
        msPts = msPts[tmp_index]
        msDia = 2 * np.power(3 * ms.bounding_sphere.volume / (4 * np.pi), 1 / 3)

        # # get pose error
        # if len(gtPoses.rotations) > 0:
        #     err_3d, err_2d = compute_pose_diff(msPts, gtPoses.K, gtPoses.rotations[0], gtPoses.translations[0], R, T, isSym)
        # else:
        #     err_3d = msDia
        #     err_2d = 100
        ...
        try:
            ...
            # cvImg = add_pose_contour(meshes[cls_id], gtPoses.K, R, T, (0,255,0), cvImg)

            # put some info on the image
            if False:
                # get Euler angles (for display only)
                ai, aj, ak = transforms3d.euler.mat2euler(R, axes='szyx')
                info = ("Euler angles: %.3f, %.3f, %.3f" % (ak, aj, ai))
                cvImg = cv2.putText(cvImg, info, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 10,
                                    cv2.LINE_AA)  # black background
                cvImg = cv2.putText(cvImg, info, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
                info = ("Translations: %.3f, %.3f, %.3f" % (T[0], T[1], T[2]))
                cvImg = cv2.putText(cvImg, info, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 10, cv2.LINE_AA)
                cvImg = cv2.putText(cvImg, info, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
                info = ("Error (ADI): %.3f / %.3f = %.2f %%" % (err_3d, msDia, 100 * err_3d / msDia))
                cvImg = cv2.putText(cvImg, info, (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 10, cv2.LINE_AA)
                cvImg = cv2.putText(cvImg, info, (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
        except:
            pass
...
def remap_pose(srcK, srcR, srcT, pt3d, dstK, transM):
    ...
    retval, rot, trans = cv2.solvePnP(pt3d.reshape(ptCnt, 1, 3), xy2d.reshape(ptCnt, 1, 2), dstK, None,
...
def pose_symmetry_handling(R, T, sym_types):
    if len(sym_types) == 0:
        return R, T
...
def get_img_from_matplotlib_fig(fig, dpi=300):
    buf = io.BytesIO()
    fig.savefig('/cvlabdata2/home/javed/quant-swisscube-only/cvlgraph.png', format="png", dpi=dpi)
    # buf.seek(0)
    ...
    for k in adi_keys:
        ...
        for i in range(depth_bins):
            if k in accuracy_adi_per_depth[i]:
                ...
                if k.find(".10d")!=-1:
                    print(accuracy_adi_per_depth[i][k])
            else:
                ys.append(0)
        ys = np.array(ys)
        #
