...
class PoseModule(nn.Module):
    def __init__(self,cfg,backbone):
        ...
        # self.head2 = PoseHead(out_channel, n_class, n_conv, prior)
        # self.head3 = PoseHead(out_channel, n_class, n_conv, prior)
        # self.head4 = PoseHead(out_channel, n_class, n_conv, prior)
    ...
    def forward(self, images, targets):
        ...
        pred_cls_ensemble, pred_reg_ensemble = [],[]
        # print(pred_cls_ensemble)
        pred_cls, pred_reg = self.head(features)
        # pred_cls2, pred_reg2 = self.head2(features)
        # pred_cls3, pred_reg3 = self.head3(features)
        # pred_cls4, pred_reg4 = self.head4(features)
        ...
        # print("regress1")
        # print((pred_reg[0]).shape)
        # print("cls2")
        # print((pred_cls2[0]).shape)
        # print("regress2")
        # print((pred_reg2[0]).shape)

        pred_cls_ensemble = pred_cls
        pred_reg_ensemble = pred_reg
        ...
        # if self.training:
        #     return self._forward_train4head(pred_cls, pred_reg,pred_cls2, pred_reg2, pred_cls3, pred_reg3, pred_cls4, pred_reg4, targets, anchors)
        # else:
        #     return self._forward_test4head(pred_cls, pred_reg,pred_cls2, pred_reg2, pred_cls3, pred_reg3, pred_cls4, pred_reg4, targets, anchors)
    ...
    def _forward_train4head(self, pred_cls, pred_reg,pred_cls2, pred_reg2,pred_cls3, pred_reg3,pred_cls4, pred_reg4, targets, anchors):

        loss_cls, loss_reg = self.loss_evaluator(
            pred_cls, pred_reg, targets, anchors
        )
        loss_cls2, loss_reg2 = self.loss_evaluator(
            pred_cls2, pred_reg2, targets, anchors
        )
        loss_cls3, loss_reg3 = self.loss_evaluator(
            pred_cls3, pred_reg3, targets, anchors
        )
        loss_cls4, loss_reg4 = self.loss_evaluator(
            pred_cls4, pred_reg4, targets, anchors
        )
        losses = {
            "loss_cls": loss_cls,
            "loss_reg": loss_reg,
    ...
    def _forward_test4head(self, pred_cls, pred_reg, pred_cls2, pred_reg2, pred_cls3, pred_reg3, pred_cls4, pred_reg4, targets, anchors):
        pred = self.post_processor(pred_cls, pred_reg, pred_cls2, pred_reg2, pred_cls3, pred_reg3, pred_cls4, pred_reg4, targets, anchors)
        # print(pred)
        return pred, {}
